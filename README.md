...
1. Spring Security를 적용했을 때 어떤 점이 도움이 되셨나요?   
    : spring security 자체적으로 보안과 관련하여 많은 옵션을 제공하고 있어 보안관련 로직을 작성하지 않아도 된다. 
2. Spring Security를 사용하지 않는다면 어떻게 인증/인가를 효율적으로 처리할 수 있을까요?  
    : Servlet Filter 를 사용 / AOP 사용 등 다양하다. 
3. AOP에 대해 설명해 주세요!  
    : 핵심 로직과 부가 기능을 분리하여 애플리케이션 전체에 걸쳐 사용되는 부가 기능을 모듈화 하여 재사용할 수 있도록 지원
...
5. 즉시로딩 / 지연로딩에 대해 설명해 주세요!  
    : 즉시 로딩은 데이터를 조회할 때 연관된 데이터까지 한번에 불러오는 것이며, 지연 로딩은 필요한 시점에 연관된 데이터를 불러오는 것
